...
func EncodeToIEDataType(dataType IEDataType, val interface{}, buff *bytes.Buffer) error {
	...
	case Ipv4Address:
		...
		if ipv4Add := v.To4(); ipv4Add != nil {
			...
			return fmt.Errorf("provided IP does not belong to IPv4 address family")
		}
	case Ipv6Address:
		// Expects net.IP type
		v, ok := val.(net.IP)
		...
		if ipv6Add := v.To16(); ipv6Add != nil {
			...
			return fmt.Errorf("provided IPv6 address is not of correct length")
		}
	case String:
		v, ok := val.(string)
		if !ok {
			return fmt.Errorf("val argument is not of type string for this element")
		}
		if len(v) < 255 {
